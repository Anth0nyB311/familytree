...
def display_calendar(family_members):
        ...
        if dob.lower() != "na":  # so if the person has a date of birth
            try:
                ...
                if person.death_date and person.death_date.lower() != "na":
                    ...
                    )  # get the death date if possible
    ...
    def get_ordinal_suffix(
        day,
    ):  # get the suffix for the day so the first day will be 1st and the second day will be 2nd
        if 11 <= day <= 13:
            return "th"
        else:
    ...
    def main(stdscr):
        ...
        try:
            while True:
                ...
                for (
                    person
                ) in important_dates:  # get the birthdays of the people in the month
                ...
                days_with_birthdays = list(
                    birthdays_in_month.keys()
                )  # get the days with birthdays
                    ...
                    if idx < height - 1:
                        ...
                        for person in birthdays_in_month[day]:  # display the person
                            if person["is_alive"]:
                                ...
                                death_month_name = calendar.month_name[
                                    person["death_date"].month
                                ]
                ...
                if (
                    key == curses.KEY_RIGHT or key == curses.KEY_DOWN
                    ...
                    key == curses.KEY_LEFT or key == curses.KEY_UP
